...
const SWAP_RATES: Record<string, Partial<Record<string, number>>> = {
  BTC: { ETH: 16.55, USDT: 96420, USDC: 96420 },
  ETH: { BTC: 0.0604, USDT: 3594.5, USDC: 3596.8 },
  USDT: { BTC: 0.00001042, ETH: 0.000278, USDC: 1.0002 },
  USDC: { BTC: 0.00001039, ETH: 0.000278, USDT: 1.0002 },
};
...
function CreateForm({ onSave, onCancel }: { onSave: (v: Swap) => void; onCancel: () => void }) {
  const [userId, setUserId] = React.useState(ALL_USERS[0]?.id ?? "");
  const [fromSym, setFromSym] = React.useState<typeof CRYPTO_ASSETS[number]>("ETH");
  const [toSym, setToSym] = React.useState<typeof CRYPTO_ASSETS[number]>("USDT");
  const [fromAmount, setFromAmount] = React.useState("");
  const [description, setDescription] = React.useState("");
  const [touched, setTouched] = React.useState(false);

  const user = ALL_USERS.find(u => u.id === userId);
  const rate = SWAP_RATES[fromSym]?.[toSym] ?? 1;
  const fromNum = parseFloat(fromAmount);
  const toAmount = isNaN(fromNum) ? 0 : Math.round(fromNum * rate * 1e6) / 1e6;
  const fee = isNaN(fromNum) ? 0 : Math.round(fromNum * 0.002 * 1e6) / 1e6;
  const amtErr = !fromAmount ? "Required" : isNaN(fromNum) || fromNum <= 0 ? "Must be positive" : null;
  const pairErr = fromSym === toSym ? "From and To assets must differ" : null;

  function handle(e: React.FormEvent) {
    e.preventDefault();
    setTouched(true);
    if (!!(amtErr || pairErr) || !user) return;
    const id = nextSwId();
    const now = new Date().toISOString();
    onSave({
      id, reference: `GG-SW-${id.slice(3)}`,
      userId: user.id, userName: user.name, userEmail: user.email,
      walletId: `wallet-${user.id.slice(0, 8)}`,
      status: "pending", assetSymbol: fromSym, assetKind: "crypto",
      amount: fromNum, fee, netAmount: fromNum - fee,
      description: description.trim() || null, failureReason: null,
      initiatedAt: now, completedAt: null, createdAt: now,
      fromSymbol: fromSym, toSymbol: toSym, fromAmount: fromNum, toAmount, rate, slippageBps: 30,
    });
  }

  const toSymOptions = CRYPTO_ASSETS.filter(a => a !== fromSym);
    ...
    <form onSubmit={handle} className="flex h-full flex-col">
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-sm font-medium text-ink">From Asset</label>
            <select value={fromSym} onChange={e => { setFromSym(e.target.value as typeof fromSym); if (e.target.value === toSym) setToSym(CRYPTO_ASSETS.find(a => a !== e.target.value)!); }} className="h-9 w-full rounded-lg border border-line bg-paper-raised px-3 text-sm text-ink">
              {CRYPTO_ASSETS.map(a => <option key={a}>{a}</option>)}
            </select>
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-ink">To Asset</label>
            <select value={toSym} onChange={e => setToSym(e.target.value as typeof toSym)} className="h-9 w-full rounded-lg border border-line bg-paper-raised px-3 text-sm text-ink">
              {toSymOptions.map(a => <option key={a}>{a}</option>)}
            </select>
          </div>
        </div>
        {touched && pairErr && <p className="text-xs text-danger">{pairErr}</p>}
        <div>
          <label className="mb-1 block text-sm font-medium text-ink">From Amount</label>
          <Input value={fromAmount} onChange={e => setFromAmount(e.target.value)} placeholder="0.1000" type="number" min="0" step="any" />
          {touched && amtErr && <p className="mt-1 text-xs text-danger">{amtErr}</p>}
        </div>
        {!isNaN(fromNum) && fromNum > 0 && (
          <div className="rounded-lg border border-line bg-paper-sunken px-3 py-2.5 text-sm">
            <p className="text-ink-muted">You receive ≈</p>
            <p className="font-mono font-semibold text-ink">{fmtCrypto(toAmount, toSym)}</p>
            <p className="mt-0.5 text-xs text-ink-subtle">Rate: 1 {fromSym} = {rate.toLocaleString()} {toSym} · Fee: {fmtCrypto(fee, fromSym)}</p>
          </div>
        )}
        <div>
...
function EditForm({ record, onSave, onCancel }: { record: Swap; onSave: (d: string | null) => void; onCancel: () => void }) {
    ...
    <form onSubmit={e => { e.preventDefault(); onSave(description.trim() || null); }} className="flex h-full flex-col">
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        <div className="flex flex-col gap-2 rounded-xl border border-line bg-paper-sunken px-4 py-3 text-sm">
          ...
          <div className="flex justify-between"><span className="text-ink-muted">Pair</span><span className="text-ink">{record.fromSymbol} → {record.toSymbol}</span></div>
          <div className="flex justify-between"><span className="text-ink-muted">From</span><span className="font-mono text-ink">{fmtCrypto(record.fromAmount, record.fromSymbol)}</span></div>
          <div className="flex justify-between"><span className="text-ink-muted">To</span><span className="font-mono text-ink">{fmtCrypto(record.toAmount, record.toSymbol)}</span></div>
          ...
          {record.slippageBps != null && <div className="flex justify-between"><span className="text-ink-muted">Slippage</span><span className="text-ink">{record.slippageBps} bps</span></div>}
...
export function SwapsManager({ initialSwaps }: { initialSwaps: Swap[] }) {
  const [records, setRecords] = React.useState(initialSwaps);
  const [search, setSearch] = React.useState("");
  const [statusFilter, setStatusFilter] = React.useState<TxnStatus | "all">("all");
  const [dateRange, setDateRange] = React.useState<DateRange | null>(null);
  const [page, setPage] = React.useState(0);
  const [createOpen, setCreateOpen] = React.useState(false);
  const [editTarget, setEditTarget] = React.useState<Swap | null>(null);
  const [actionTarget, setActionTarget] = React.useState<{ record: Swap; action: "approve" | "reject" | "cancel" | "reverse" } | null>(null);
  ...
  function handleCreate(v: Swap) { setRecords(r => [v, ...r]); setCreateOpen(false); }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <KpiCard title="Total Swaps" value={summary.total} />
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search reference, user, pair…" className="pl-9" />
        </div>
        ...
        <Button onClick={() => setCreateOpen(true)}><Icons.plus className="size-4" />New swap</Button>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-line">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-line">
            {paged.length === 0 && <tr><td colSpan={9} className="py-12 text-center text-ink-subtle">No swaps found</td></tr>}
            {paged.map(r => (
              <tr key={r.id} className="transition-colors hover:bg-paper-sunken/50">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    <Badge tone="neutral">{r.fromSymbol}</Badge>
                    <Icons.swap className="size-3 text-ink-subtle" />
                    <Badge tone="neutral">{r.toSymbol}</Badge>
                  </div>
                </td>
                <td className="px-4 py-3 text-right font-mono text-xs text-ink">{fmtCrypto(r.fromAmount, r.fromSymbol)}</td>
                <td className="px-4 py-3 text-right font-mono text-xs text-ink">{fmtCrypto(r.toAmount, r.toSymbol)}</td>
                <td className="px-4 py-3 text-right font-mono text-xs text-ink-muted">{r.rate.toLocaleString()}</td>
      ...
      <Sheet open={createOpen} onClose={() => setCreateOpen(false)} title="New Swap" description="Execute a crypto-to-crypto swap">
        <CreateForm onSave={handleCreate} onCancel={() => setCreateOpen(false)} />
      </Sheet>
      <Sheet open={!!editTarget} onClose={() => setEditTarget(null)} title="Edit Swap" description={editTarget?.reference}>
