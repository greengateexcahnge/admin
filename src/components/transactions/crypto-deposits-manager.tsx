...
import {
  type CryptoDeposit, type TxnStatus,
...
const CRYPTO_NETWORKS: Record<string, { code: string; name: string; required: number }[]> = {
  BTC: [{ code: "bitcoin", name: "Bitcoin", required: 3 }],
  ETH: [{ code: "ethereum", name: "Ethereum", required: 12 }],
  USDT: [{ code: "tron", name: "TRON (TRC-20)", required: 20 }, { code: "ethereum", name: "Ethereum (ERC-20)", required: 12 }],
  USDC: [{ code: "ethereum", name: "Ethereum (ERC-20)", required: 12 }, { code: "base", name: "Base", required: 15 }],
};
...
function CreateForm({ onSave, onCancel }: { onSave: (v: CryptoDeposit) => void; onCancel: () => void }) {
  ...
  const hashErr = !txHash.trim() ? "Required" : null;
  const hasErr = !!(amtErr || hashErr);
  ...
  function handle(e: React.FormEvent) {
    ...
    onSave({
      ...
      depositAddress: "manual-entry",
      txHash: txHash.trim(),
      fromAddress: fromAddress.trim() || null,
      confirmations: 0, requiredConfirmations: net.required, blockHeight: null,
    });
  }
    ...
    <form onSubmit={handle} className="flex h-full flex-col">
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        ...
        <div>
          ...
          <Input value={amount} onChange={e => setAmount(e.target.value)} placeholder={assetSym === "BTC" ? "0.00500000" : "10.0000"} type="number" min="0" step="any" />
          {touched && amtErr && <p className="mt-1 text-xs text-danger">{amtErr}</p>}
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-ink">Transaction Hash</label>
          <Input value={txHash} onChange={e => setTxHash(e.target.value)} placeholder="0x… or plain hex" className="font-mono text-xs" />
          {touched && hashErr && <p className="mt-1 text-xs text-danger">{hashErr}</p>}
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-ink">From Address <span className="font-normal text-ink-subtle">(optional)</span></label>
          <Input value={fromAddress} onChange={e => setFromAddress(e.target.value)} placeholder="Sender address" className="font-mono text-xs" />
        </div>
        <div>
...
function EditForm({ record, onSave, onCancel }: { record: CryptoDeposit; onSave: (d: string | null) => void; onCancel: () => void }) {
    ...
    <form onSubmit={e => { e.preventDefault(); onSave(description.trim() || null); }} className="flex h-full flex-col">
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        <div className="flex flex-col gap-2 rounded-xl border border-line bg-paper-sunken px-4 py-3 text-sm">
          ...
          <div className="flex flex-col gap-1"><span className="text-ink-muted">TX Hash</span><span className="font-mono text-xs text-ink break-all">{record.txHash}</span></div>
          {record.fromAddress && <div className="flex flex-col gap-1"><span className="text-ink-muted">From</span><span className="font-mono text-xs text-ink break-all">{record.fromAddress}</span></div>}
          <div className="flex justify-between"><span className="text-ink-muted">Confirmations</span><span className="text-ink">{record.confirmations} / {record.requiredConfirmations}</span></div>
...
export function CryptoDepositsManager({ initialDeposits }: { initialDeposits: CryptoDeposit[] }) {
  ...
  const [editTarget, setEditTarget] = React.useState<CryptoDeposit | null>(null);
  const [actionTarget, setActionTarget] = React.useState<{ record: CryptoDeposit; action: "approve" | "reject" | "cancel" } | null>(null);
  ...
  const filtered = records.filter(r => {
    ...
    if (q && !r.reference.toLowerCase().includes(q) && !r.userName.toLowerCase().includes(q) && !r.txHash.toLowerCase().includes(q) && !r.networkName.toLowerCase().includes(q)) return false;
  ...
  function handleCreate(v: CryptoDeposit) { setRecords(r => [v, ...r]); setCreateOpen(false); }
  ...
  function handleAction() {
    ...
    setRecords(r => r.map(x => {
      ...
      if (action === "approve") return { ...x, status: "completed" as TxnStatus, completedAt: now, confirmations: x.requiredConfirmations };
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search reference, user, tx hash…" className="pl-9" />
        </div>
        ...
        <Button onClick={() => setCreateOpen(true)}><Icons.plus className="size-4" />New deposit</Button>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-line">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-line">
            {paged.length === 0 && <tr><td colSpan={8} className="py-12 text-center text-ink-subtle">No crypto deposits found</td></tr>}
            {paged.map(r => (
              <tr key={r.id} className="transition-colors hover:bg-paper-sunken/50">
                ...
                <td className="px-4 py-3"><div className="flex items-center gap-1.5"><Badge tone="neutral">{r.assetSymbol}</Badge></div><p className="mt-0.5 text-xs text-ink-muted">{r.networkName}</p></td>
                <td className="px-4 py-3 text-right font-mono text-sm font-medium text-ink">{fmtCrypto(r.amount, r.assetSymbol)}</td>
                <td className="px-4 py-3 font-mono text-xs text-ink-muted">{r.txHash.slice(0, 10)}…{r.txHash.slice(-6)}</td>
                <td className="px-4 py-3">
                  <span className={cn("text-sm font-medium", r.confirmations >= r.requiredConfirmations ? "text-success" : "text-warning")}>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    {canApprove(r.status) && <button onClick={() => setActionTarget({ record: r, action: "approve" })} title="Confirm deposit" className="grid size-7 place-items-center rounded-md text-ink-muted hover:bg-success/10 hover:text-success"><Icons.checkCircle className="size-4" /></button>}
      ...
      <Sheet open={createOpen} onClose={() => setCreateOpen(false)} title="New Crypto Deposit" description="Manually record an on-chain credit">
      ...
      {actionTarget && actionTarget.action !== "reject" && (
        <Dialog open onClose={() => setActionTarget(null)} tone={actionTarget.action === "cancel" ? "warning" : "neutral"}
          title={actionTarget.action === "approve" ? "Confirm Deposit" : "Cancel Deposit"}
          description={actionTarget.action === "approve" ? `Manually confirm ${actionTarget.record.reference} (${fmtCrypto(actionTarget.record.amount, actionTarget.record.assetSymbol)})?` : `Cancel ${actionTarget.record.reference}?`}
          footer={<div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setActionTarget(null)}>No, keep it</Button><Button variant={actionTarget.action === "approve" ? "primary" : "ghost"} className={cn(actionTarget.action !== "approve" && "text-warning hover:bg-warning/10 hover:text-warning")} onClick={handleAction}>{actionTarget.action === "approve" ? "Confirm" : "Cancel deposit"}</Button></div>}
        />
      )}
      {actionTarget?.action === "reject" && (
        <Dialog open onClose={() => setActionTarget(null)} tone="danger" title="Reject Deposit" description={`Reject ${actionTarget.record.reference}?`}
