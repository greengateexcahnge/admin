...
import {
  type FiatWithdrawal, type TxnStatus,
...
const FW_PROVIDERS = ["paystack", "flutterwave", "lenco", "manual"];
const BANKS = [
...
function CreateForm({ onSave, onCancel }: { onSave: (v: FiatWithdrawal) => void; onCancel: () => void }) {
  const [userId, setUserId] = React.useState(ALL_USERS[0]?.id ?? "");
  const [amount, setAmount] = React.useState("");
  const [accountName, setAccountName] = React.useState("");
  const [accountNumber, setAccountNumber] = React.useState("");
  const [bankIdx, setBankIdx] = React.useState(0);
  const [provider, setProvider] = React.useState(FW_PROVIDERS[0]);
  ...
  const nameErr = !accountName.trim() ? "Required" : null;
  const numErr = !accountNumber.trim() ? "Required" : !/^\d{10}$/.test(accountNumber) ? "Must be 10 digits" : null;
  ...
  function handle(e: React.FormEvent) {
    ...
    onSave({
      id,
      reference: `GG-FW-${id.slice(3)}`,
      userId: user.id, userName: user.name, userEmail: user.email,
      walletId: `wallet-${user.id.slice(0, 8)}`,
      status: "pending", assetSymbol: "NGN", assetKind: "fiat",
      amount: amountNum, fee: 52.5, netAmount: amountNum - 52.5,
      description: description.trim() || null, failureReason: null,
      initiatedAt: now, completedAt: null, createdAt: now,
      accountNumber: accountNumber.trim(), accountName: accountName.trim().toUpperCase(),
      bankName: bank.name, bankCode: bank.code,
      provider: provider ?? "manual", providerRef: null, nipSessionId: null,
    });
  }
    ...
    <form onSubmit={handle} className="flex h-full flex-col">
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        ...
        <div>
          ...
          <Input value={amount} onChange={e => setAmount(e.target.value)} placeholder="100000.00" type="number" min="1" step="0.01" />
          {touched && amtErr && <p className="mt-1 text-xs text-danger">{amtErr}</p>}
          <p className="mt-0.5 text-xs text-ink-subtle">Flat fee: ₦52.50</p>
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-ink">Beneficiary Account Name</label>
          <Input value={accountName} onChange={e => setAccountName(e.target.value)} placeholder="JOHN ADAMU" />
          {touched && nameErr && <p className="mt-1 text-xs text-danger">{nameErr}</p>}
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-ink">Beneficiary Account Number</label>
        ...
        <div>
          ...
          <select value={bankIdx} onChange={e => setBankIdx(Number(e.target.value))} className="h-9 w-full rounded-lg border border-line bg-paper-raised px-3 text-sm text-ink">
            {BANKS.map((b, i) => <option key={b.code} value={i}>{b.name}</option>)}
          </select>
        </div>
        <div>
          ...
          <select value={provider} onChange={e => setProvider(e.target.value)} className="h-9 w-full rounded-lg border border-line bg-paper-raised px-3 text-sm text-ink capitalize">
            {FW_PROVIDERS.map(p => <option key={p} className="capitalize">{p}</option>)}
          </select>
        </div>
        <div>
...
function EditForm({ record, onSave, onCancel }: { record: FiatWithdrawal; onSave: (d: string | null) => void; onCancel: () => void }) {
    ...
    <form onSubmit={e => { e.preventDefault(); onSave(description.trim() || null); }} className="flex h-full flex-col">
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        <div className="flex flex-col gap-2 rounded-xl border border-line bg-paper-sunken px-4 py-3 text-sm">
          ...
          <div className="flex justify-between"><span className="text-ink-muted">Beneficiary</span><span className="text-ink">{record.accountName}</span></div>
          <div className="flex justify-between"><span className="text-ink-muted">Account No.</span><span className="font-mono text-ink">{record.accountNumber}</span></div>
          <div className="flex justify-between"><span className="text-ink-muted">Bank</span><span className="text-ink">{record.bankName}</span></div>
          ...
          {record.nipSessionId && <div className="flex justify-between"><span className="text-ink-muted">NIP Session</span><span className="font-mono text-xs text-ink-subtle">{record.nipSessionId}</span></div>}
...
export function FiatWithdrawalsManager({ initialWithdrawals }: { initialWithdrawals: FiatWithdrawal[] }) {
  ...
  const [editTarget, setEditTarget] = React.useState<FiatWithdrawal | null>(null);
  const [actionTarget, setActionTarget] = React.useState<{ record: FiatWithdrawal; action: "approve" | "reject" | "cancel" | "reverse" } | null>(null);
  ...
  const filtered = records.filter(r => {
    ...
    if (q && !r.reference.toLowerCase().includes(q) && !r.userName.toLowerCase().includes(q) && !r.accountName.toLowerCase().includes(q) && !r.bankName.toLowerCase().includes(q)) return false;
  ...
  function handleCreate(v: FiatWithdrawal) { setRecords(r => [v, ...r]); setCreateOpen(false); }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="overflow-x-auto rounded-xl border border-line">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-line">
            {paged.length === 0 && <tr><td colSpan={8} className="py-12 text-center text-ink-subtle">No withdrawals found</td></tr>}
            {paged.map(r => (
              <tr key={r.id} className="transition-colors hover:bg-paper-sunken/50">
                ...
                <td className="px-4 py-3"><p className="text-ink">{r.accountName}</p><p className="font-mono text-xs text-ink-muted">{r.accountNumber} · {r.bankName}</p></td>
      ...
      <Sheet open={createOpen} onClose={() => setCreateOpen(false)} title="New Fiat Withdrawal" description="Initiate a manual payout">
